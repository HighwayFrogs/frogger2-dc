	...
	if(checkForSoftReset())
	{
		...
		if (saveInfo.saveStage != SAVEMENU_SAVE)
		{
			if ((gameState.mode != FRONTEND_MODE) && (quittingLevel == FALSE))
			{
				...
				// ma - for when you quit out in multiplayer menu mode
...
//	PrepareSong(AUDIOTRK_LEVELCOMPLETE,NO);
	...
	if(garibList.maxCoins)
	{
		...
		{
			// ma - adjust text position for foriegn language version
			coinText = CreateAndAddTextOverlay(2048+4096+1024,850,GAMESTRING(STR_GOT_ALL_COINS),YES,255,font,TEXTOVERLAY_SHADOW);
	...
	{
		grade = 3;
		// ma - increased start position to stop poking on screen in foreign language version
		coinText = CreateAndAddTextOverlay(2048+4096+1024,980+300,GAMESTRING(STR_TRAINING_COMPLETE),YES,255,font,TEXTOVERLAY_SHADOW);
		coinText->xPosTo = 2048;
	}
	else
	{
	...
	{
		// ma - increased start position to stop poking on screen in foreign language version
		newBestText = CreateAndAddTextOverlay(2048+4096+256,850+800,GAMESTRING(STR_NEW_BEST_TIME),YES,255,font,TEXTOVERLAY_SHADOW);
		newBestText->r = 255;
		newBestText->g = 255;
		newBestText->b = 0;
	...
	{
		// ma - increased start position to stop poking on screen in foreign language version
		tText = CreateAndAddTextOverlay(2048+4096+256, 850, levTimeText, YES, (char)0xFF, font, TEXTOVERLAY_SHADOW);
		tText->xPosTo = 2048;
		bIcon = CreateAndAddSpriteOverlay(2048-256+4096,1150,trophyName[grade],512,512,255,0);
		bIcon->xPosTo = 2048 - 256;
		oldBestText = CreateAndAddTextOverlay(2048 + 4096 + 256,1950,oldBestStr,YES,255,font,TEXTOVERLAY_SHADOW);
	...
	{
		// ma - increased start position to stop poking on screen in foreign language version
		oText[0] = CreateAndAddTextOverlay(2048+4096+1024, 3420, GAMESTRING(STR_PRESS_X_TO_CONTINUE), YES, (char)0xFF, font, TEXTOVERLAY_SHADOW);
		oText[0]->xPosTo = 2048;
		oText[1] = NULL;
	}
	else
	{
		// ma - increased start position to stop poking on screen in foreign language version
		oText[0] = CreateAndAddTextOverlay(2048+4096+256, 3220, GAMESTRING(STR_RESTARTLEVEL), YES, (char)0xFF, font, TEXTOVERLAY_SHADOW);
		oText[1] = CreateAndAddTextOverlay(2048-4096-256, 3610, GAMESTRING(STR_QUIT), YES, (char)0xFF, font, TEXTOVERLAY_SHADOW);
		oText[0]->xPosTo = 2048;
		oText[1]->xPosTo = 2048;
	}
	nText = NULL;
	...
	if (grade==0)
	{
		...
		// ma -  frig to get arrows in the right place
		if((gameTextLang == LANG_US)||(gameTextLang == LANG_UK))
		{
			nText->xPosTo = 2048 - w;// + 256;
		}
		else
		{
			nText->xPosTo = 2048 - w + 256;
...
//					enterNameButton[3]->xPos = nText->xPosTo + w + 40 + fontExtentWScaled(nText->font,textString,4096)*8;
				...
				if(textEntry == 0)
				{
					...
					if(grade==0)
					{
						strcpy((char *)worldVisualData[player[0].worldNum].levelVisualData[player[0].levelNum].parName,textString);
