...
//		if((numSfxPlaying() < MAX_AMBIENT_SAMPLES)/*&&(sampleWaitCounter > AMBIENT_WAIT)*/)
		{
//ma			sfxPrintf("sfxPlaySample recalled.. id%s, vl%d, vr%d, ph%d", sample->sample->idName, vl, vr, pitch);

	 		lastSound = sample->handle = sfxPlaySample(sample->sample->snd, vl,vr, pitch);

//ma			sfxPrintf("sfxPlaySample recalled returned.. %d", lastSound);
		}
//		else
//		{
//			return -1;
 ...
 */
 ...
 //		else	//PUT BACK IN?!?!
		...
		{	
//ma			if((numSfxPlaying() < MAX_AMBIENT_SAMPLES)/*&&(sampleWaitCounter > AMBIENT_WAIT)*/)
			{
